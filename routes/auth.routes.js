    ...
    async (req, res) => {
        try {
            ...
            if (!errors.isEmpty()) {
                return res.status(400).json({
                    errors: errors.array(),
                    massage: 'Некорректные данные при входу в систему '
                })
            }
            ...
            if (!user) {
                return res.status(400).json({message: 'Пользоватьль не найден'})
            }

            // смотрит совпадает ли пароль найденого пользователя
